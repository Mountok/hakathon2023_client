  ...
  return (
    <div className="events_block">
      <div className="events_block_header">
        <p>Получите незабываемые эмоции</p>
        <p>
          от <span>СОБЫТИЙ НЕДЕЛИ</span>!
        </p>
      </div>
        ...
        {currentState.length != 0 ? (
          <>
            <div key={currentState.at(-1).event_id} className="card_item">
              <img
                src={`${currentState.at(-1).event_image}`}
                alt="#"
              ...
              <button
                onClick={() =>
                  navigate(`/event?id=${currentState.at(-1).event_id}`)
                }
              >
                ЗАПИСАТЬСЯ
              </button>
            ...
            <div key={currentState.at(-2).event_id} className="card_item">
              <img
                src={currentState.at(-2).event_image}
                alt="#"
                className="card_item_bg"
              />
              <div className="card_item_header">
              ...
              <button
                onClick={() =>
                  navigate(`/event?id=${currentState.at(-2).event_id}`)
                }
              >
                ЗАПИСАТЬСЯ
              </button>
            ...
            <div key={currentState.at(-3).event_id} className="card_item">
              <img
                src={currentState.at(-3).event_image}
                alt="#"
                className="card_item_bg"
              />
              <div className="card_item_header">
              ...
              <button
                onClick={() =>
                  navigate(`/event?id=${currentState.at(-3).event_id}`)
                }
              >
                ЗАПИСАТЬСЯ
              </button>
