...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    let passIsTrue = false, loginIsTrue = false;
    if(/[a-zA-Z]/.test(userPassword.charAt(0))){
      if(/\d/.test(userPassword)){
        if(userPassword.length > 3){
          passIsTrue = true
        }else 
          alert('Длина пароля меньше 3-х символоф')
       }
        else 
          alert('Пароль пароль должен содержать хотябы одну цифру')
    }else
      alert('Пароль неможет начинаться с цифры')
